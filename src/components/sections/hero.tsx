...
export function HeroSection({ title, subtitle, cta, cta2, image }: HeroProps) {
  const [ctaText, ctaLink] = cta?.split('|') ?? ['Get Started', '#contact']
  const [cta2Text, cta2Link] = cta2?.split('|') ?? []

  return (
    <section className="relative min-h-[92vh] flex items-center justify-center overflow-hidden" style={{ background: 'hsl(222, 47%, 8%)' }}>
      {/* Background image */}
      {image && (
        <Image
          src={image}
      ...
      {/* Dark gradient overlay to blend image into dark theme */}
      <div className="absolute inset-0" style={{ zIndex: 1, background: 'linear-gradient(to bottom, rgba(10,15,30,0.5) 0%, rgba(10,15,30,0.3) 50%, rgba(10,15,30,0.7) 100%)' }} />
      {/* Grid overlay */}
      <div className="absolute inset-0 bg-grid-pattern opacity-30" style={{ zIndex: 2 }} />
      {/* Gradient orbs for depth */}
      <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full" style={{ zIndex: 2, background: 'radial-gradient(circle, hsl(221, 83%, 55%) 0%, transparent 70%)', opacity: 0.12 }} />
      <div className="absolute bottom-1/4 right-1/4 w-64 h-64 rounded-full" style={{ zIndex: 2, background: 'radial-gradient(circle, hsl(260, 70%, 50%) 0%, transparent 70%)', opacity: 0.08 }} />
